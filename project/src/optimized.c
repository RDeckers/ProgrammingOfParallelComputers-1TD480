...
int main(int argc, char **argv){
  ...
  mem_spin = clCreateBuffer(context, CL_MEM_READ_WRITE, dimensions[0]*dimensions[1]*sizeof(cl_int), NULL, &ret);
  if(CL_SUCCESS != ret){
  ...
  mem_energy = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dimensions[0]*dimensions[1]*sizeof(cl_int), NULL, &ret);
  if(CL_SUCCESS != ret){
  ...
  cl_int *spin_field = malloc(sizeof(cl_int)*dimensions[0]*dimensions[1]);
  if(NULL == spin_field){
    report(FAIL, "Allocation failure: %s (%d)", strerror(errno), errno);
    return -1;
  }
  ...
  if(CL_SUCCESS != (ret = clEnqueueWriteBuffer(com_qs[0], mem_spin, CL_FALSE, 0, dimensions[0]*dimensions[1] * sizeof(cl_float), spin_field, 0, NULL, NULL))){
    report(FAIL, "enqueue write spins returned: %s (%d)",cluErrorString(ret), ret);
  }
  if(CL_SUCCESS != (ret = clEnqueueWriteBuffer(com_qs[0], mem_chances, CL_FALSE, 0, 10 * sizeof(cl_uint), acceptance_chances, 0, NULL, NULL))){
  ...
  //upload rng[0]
  if(CL_SUCCESS != (ret = clEnqueueWriteBuffer(com_qs[0], mem_rng, CL_FALSE, 0, dimensions[0]*dimensions[1] * sizeof(cl_int), rng_field, 0, NULL, NULL))){
...
///////////////////////////////////////////////*/
  ...
  ret = clEnqueueReadBuffer(com_qs[0], mem_energy, CL_TRUE, 0,dimensions[0]*dimensions[1]*sizeof(cl_int), energy_field, 0, NULL, NULL);
  if(CL_SUCCESS != ret){
