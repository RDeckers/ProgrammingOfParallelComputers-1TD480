...
/*
	The output is what I would expect. because each thread only a data[tid] the final result is always the same.
	The intermediate sum changes depending on which thread has performed which part of its operation before a lock is obtained.
	GlobData and sum can even change in between computing sum and printing GlobData meaning the answer looks off.
*/
